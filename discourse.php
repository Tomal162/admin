...
session_start();
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_FILES['discourse_image'])) {
    ...
    $target_file = $target_dir . basename($_FILES["discourse_image"]["name"]);
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));

    // Check file size (optional, e.g., limit to 500KB)
    if ($_FILES["discourse_image"]["size"] > 500000) {
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["discourse_image"]["tmp_name"], $target_file)) {
            // Prepare data for insertion
            $discourse_image = basename($_FILES["discourse_image"]["name"]);
            $discourse_head = mysqli_real_escape_string($con, $_POST['discourse_head']);
            $discourse_writer = mysqli_real_escape_string($con, $_POST['discourse_writer']);
            $discourse_date = mysqli_real_escape_string($con, $_POST['discourse_date']);
            $discourse_content = mysqli_real_escape_string($con, $_POST['discourse_content']);
            
            // Insert the data into the database
            $query = "insert into discourse (discourse_image, discourse_head, discourse_writer, discourse_date, discourse_content) 
                      values ('$discourse_image', '$discourse_head', '$discourse_writer', '$discourse_date', '$discourse_content')";
    
            if(mysqli_query($con, $query)) {
                header("Location: discourse.php"); // Redirect to prevent form resubmission
                exit;
            } else {
    ...
    <main class="w-full md:w-[calc(100%-256px)] md:ml-64 bg-gray-50 min-h-screen transition-all main">
        <div class="py-2 px-6 bg-white flex items-center shadow-md shadow-black/5 sticky top-0 left-0 z-30">
            ...
            <ul class="flex items-center text-sm ml-4">
                <li class="mr-2">
                    <a href="#" class="text-gray-400 hover:text-gray-600 font-medium">Discourse</a>
                </li>
            </ul>
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                <h2 class="text-xl font-semibold mb-4">Update Discourse</h2>
                <form method="post" enctype="multipart/form-data" onsubmit="showNotification(event, 'discourse-notification')">
                <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700">Select Image</label>
                            <input type="file" name="discourse_image" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                        </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Article Headline</label>
                        <input type="text" name="discourse_head" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                    <div class="mb-4 flex space-x-4">
                        <div class="flex-1">
                            <label class="block text-sm font-medium text-gray-700">Writer Name</label>
                            <input type="text" name="discourse_writer" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        </div>
                        <div class="flex-1">
                            <label class="block text-sm font-medium text-gray-700">Date and Time</label>
                            <input type="datetime-local" name="discourse_date" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        </div>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Details</label>
                        <textarea name="discourse_content" rows="4" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"></textarea>
                    </div>
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-700">Update</button>
                </form>
                <div id="discourse-notification" class="hidden bg-green-500 text-white p-2 mt-4 rounded">Updated Successfully</div>
            </div>
